...
async function save() {
  ...
  try {
    ...
    const response = await fetch(`${normalizeInstanceUrl(config.instanceUrl)}/api/v1/statuses/${encodeURIComponent(statusId())}`, {
      ...
      body: JSON.stringify({ status: buildStatusContent() }),
    })

    if (!response.ok) throw new Error('Não foi possível salvar as alterações.')
    await Swal.fire({ icon: 'success', title: 'Salvo', text: 'Alterações salvas.', confirmButtonText: 'OK' })
...
<template>
  <main class="page edit-page">
    <div class="center edit-container">
      ...
      <div v-if="loading" class="card edit-card">Carregando...</div>
      <section v-else class="card edit-card">
        <form class="edit-form" @submit.prevent="save">
          <div class="field-group"><label>Título</label><input v-model="form.title" required /></div>
          <div class="field-group"><label>Descrição</label><textarea v-model="form.description" rows="5" required /></div>
          <div class="form-grid">
            <div class="field-group"><label>Área/categoria</label><input v-model="form.area" /></div>
            <div class="field-group"><label>Tipo</label><input v-model="form.type" /></div>
            <div class="field-group"><label>Licença</label><input v-model="form.license" /></div>
            <div class="field-group"><label>Tags</label><input v-model="form.tags" /></div>
          </div>
          <div v-if="error" class="status bad">{{ error }}</div>
          <div class="form-actions">
            <button class="btn primary" type="submit" :disabled="saving">{{ saving ? 'Salvando...' : 'Salvar alterações' }}</button>
